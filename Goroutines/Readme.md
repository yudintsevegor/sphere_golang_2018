...
Примечания:
* Через fmt.Sprintf в строку конвертировать не надо! Для этого есть пакет strconv
* select в этом задании не используется
* time.Sleep в коде быть не должно, это считается хардкодом, который запрещен
* постарайтесь свести к минимому magic numbers
